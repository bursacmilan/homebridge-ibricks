...
export class IbricksDirectorPlatformAccessory extends InternalPlatformAccessory {
    ...
    constructor(
        ...
        this.getCelloEvents().subscribe(celloEvent => {
            this._director.currentTemperature = this.isRightDevice()
                ? celloEvent.cello.currentTemperatureRight
                : celloEvent.cello.currentTemperatureLeft;

            this._director.targetTemperature = this.isRightDevice()
                ? celloEvent.cello.targetTemperatureRight
                : celloEvent.cello.targetTemperatureLeft;
    ...
    protected subscribeToCharacteristics(): void {
        this.service.getCharacteristic(this.platform.Characteristic.CurrentTemperature).onGet(this._getCurrentTemperature.bind(this));

        this.service.getCharacteristic(this.platform.Characteristic.TemperatureDisplayUnits).onGet(() => 0);
        ...
        this.service.getCharacteristic(this.platform.Characteristic.CurrentHeaterCoolerState).onGet(() => 3);

        this.service.getCharacteristic(this.platform.Characteristic.TargetHeatingCoolingState).onGet(() => 3);
    }

    protected setInitialLocalValues(): void {
        this._director.currentTemperature =
            this._director.leftRight === 1 ? this._director.cello.currentTemperatureRight : this._director.cello.currentTemperatureLeft;

        this._director.targetTemperature =
            this._director.leftRight === 1 ? this._director.cello.targetTemperatureRight : this._director.cello.targetTemperatureLeft;
    }
