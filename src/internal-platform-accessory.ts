...
export abstract class InternalPlatformAccessory {
  ...
  protected constructor(
    ...
    accessoryInformationService.setCharacteristic(this.platform.Characteristic.Manufacturer, 'iBricks')
  ...
  protected updateCharacteristic<T extends WithUUID<{
    new(): Characteristic;
  ...
  protected checkIsThisDevice(celloEvent: CelloEvent): boolean {
    return !(celloEvent.deviceType !== this._device.deviceType ||
      celloEvent.cello.mac !== this._device.mac || celloEvent.leftRight !== this._device.leftRight);
  }
