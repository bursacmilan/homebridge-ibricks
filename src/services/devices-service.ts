...
export class DevicesService {
  ...
  public getAllMeteos(): Director[] {
    this._loggerService.logDebug('getAllMeteos',
      `Getting all meteos from total of ${this._cellos.length} cellos`);
  ...
  public getAllDirectors(filterDisabled: boolean): Director[] {
    this._loggerService.logDebug('getAllDirectors',
      `Getting all directors from total of ${this._cellos.length} cellos`);

    const directors = this._getAllOfT<Director>((cello, leftRight) =>
      new Director(this._getDirectorId(cello, leftRight), cello.mac,
  ...
  public getAllShutters(): Shutter[] {
    this._loggerService.logDebug('getAllShutters',
      `Getting all shutters from total of ${this._cellos.length} cellos`);

    const shutters = this._getAllOfT<Shutter>((cello, leftRight) =>
      new Shutter(this._getShutterId(cello, leftRight), cello.mac,
        `${cello.description} - ${leftRight === 1 ? 'Right' : 'Left'}`,
  ...
  public getAllRelays(): Relay[] {
    this._loggerService.logDebug('getAllRelays',
      `Getting all relays from total of ${this._cellos.length} cellos`);

    const relays = this._getAllOfT<Relay>((cello, leftRight) =>
      new Relay(this._getRelayId(cello, leftRight), cello.mac,
        `${cello.description} - ${leftRight === 1 ? 'Right' : 'Left'}`,
  ...
  private _getAllOfT<T>(factory: (cello: Cello, leftRight: number) => T, deviceType: DeviceType, hardwareInfoProperty: string): T[] {
    this._loggerService.logDebug('_getAllOfT',
      `Getting all ${deviceType} from total of ${this._cellos.length} cellos`);

    const devices: T[] = [];
    for (const cello of this._cellos) {
      if (cello.hardwareInfo === undefined) {
        this._loggerService.logWarning('_getAllOfT',
