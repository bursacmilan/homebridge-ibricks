...
export class IbricksLightPlatformAccessory extends InternalPlatformAccessory {
    ...
    private _handleDimmer(celloEvent: CelloEvent, platform: IbricksPlatform): void {
        ...
        platform.log.debug(
            `Relay ${this._relay.name} changed to ${this._relay.isOn ? 'on' : 'off'} and brightness ${this._relay.brightness}`,
        );
    ...
    protected subscribeToCharacteristics(): void {
        this.service.getCharacteristic(this.platform.Characteristic.On).onSet(this._setIsOn.bind(this)).onGet(this._getIsOn.bind(this));
    ...
    private _setBrightness(value: CharacteristicValue): void {
        ...
        this._messageGenerator.setDimmer(
            this._relay.cello,
            this._relay.leftRight,
            this._relay.convertBrightnessForCelloValue(this._relay.brightness),
        );
    }
